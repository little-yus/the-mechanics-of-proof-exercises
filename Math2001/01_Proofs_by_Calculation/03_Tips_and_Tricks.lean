...
example {a b : ℚ} (h1 : a - 3 = 2 * b) : a ^ 2 - a + 3 = 4 * b ^ 2 + 10 * b + 9 :=
  calc
    a ^ 2 - a + 3 = a ^ 2 - 2 * (1 / 2) * a + (1 / 2) ^ 2 - (1 / 2) ^ 2 + 3 := by ring
    _ = (a - (1 / 2)) ^ 2 - (1 / 2) ^ 2 + 3 := by ring
    _ = (a - 3 + 3 - 1 / 2) ^ 2 - (1 / 2) ^ 2 + 3 := by ring
    _ = (2 * b + 3 - 1 / 2) ^ 2 - (1 / 2) ^ 2 + 3 := by rw [h1]
    _ = 4 * b ^ 2 + 2 * (5 / 2) * (2 * b) + (5 / 2) ^ 2  - (1 / 2) ^ 2 + 3 := by ring
    _ = 4 * b ^ 2 + 10 * b + 6 + 3 := by ring
    _ = 4 * b ^ 2 + 10 * b + 9 := by ring

-- Example 1.3.10
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = (z ^ 2) ^ 2 - (z ^ 2) - z * (z ^ 2 - 2) + 1 := by ring
...
example {x y : ℚ} (h1 : y + 1 = 3) (h2 : x + 2 * y = 3) : x = -1 :=
  calc
    x = -2 * (y + 1) + (x + 2 * y) + 2 := by ring
    _ = -2 * 3 + 3 + 2 := by rw [h1, h2]
    _ = -1 := by ring
